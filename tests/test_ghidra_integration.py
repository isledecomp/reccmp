...
def test_experiment_decomp_output(ghidra: "FlatProgramAPI"):
    ...
    orig_address = 0x00402880  # readIntFromRegistry()
    address_ghidra = ghidra.getAddressFactory().getAddress(hex(orig_address))

    # TODO experiment: overwrite memory
    listing = ghidra.getCurrentProgram().getListing()
    listing.clearCodeUnits(address_ghidra, address_ghidra, False)
    mem = ghidra.getCurrentProgram().getMemory()
    mem.setByte(address_ghidra, -61)  # 0xc3; TODO needs automation
    ...
    func = ghidra.getFunctionContaining(address_ghidra)
    if not func:
        print("No function at current address")
        exit()

    function_importer.overwrite_ghidra_function(func)

    # Verification

    iface = DecompInterface()
    iface.openProgram(ghidra.getCurrentProgram())

    res = iface.decompileFunction(func, 30, ghidra.getMonitor())
    if not res.decompileCompleted():
        print("Decompilation failed")
        exit()

    decompiled_c_code = res.getDecompiledFunction().getC()

    assert decompiled_c_code == """
void MyTestFn(void)
