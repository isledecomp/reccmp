...
def test_option_no_placeholders():
    ...
    def name_lookup(addr: int, *_, **__) -> str | None:
        return "Hello" if addr == 0x2000 else None

    p = ParseAsm(addr_test=addr_test, name_lookup=name_lookup)
    p.options.use_placeholders = False
