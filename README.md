...
At the moment, C++ compiled with old versions of MSVC (like 4.20) is supported. Work on support for newer MSVC versions is in progress - testing and bug reports are greatly appreciated. Other compilers and languages are not supported at the moment, but feel free to contribute if you wish to do so!
...
4. Run `reccmp-project.exe detect --search-path path/to/folder/with/original/binaries`.
5. If there is no `reccmp-build.yml` after building: Navigate to the recompiled binaries folder and run `reccmp-project.exe detect --what recompiled`.
6. Look into `reccmp-project.yml` to see what the target is called.
7. Run `reccmp-reccmp.exe --target <YOURTARGET>`. You should see a list of functions and others together with their match percentage.

### Existing decompilation project that has not used `reccmp` before

1. (Recommended) Set up and activate a virtual Python environment in the directory of your recompilation project (this is different for different operating systems and shells)
2. Install `reccmp`: `pip install https://github.com/isledecomp/reccmp`
3. Run `reccmp-project.exe create --originals path/to/original.exe --scm`. This generates two files `reccmp-project.yml` and `reccmp-user.yml`; the latter will automatically be added to the `.gitignore`.
...
4. Navigate to your recompiled binary and run `reccmp-project.exe detect --what recompiled`. A file `reccmp-build.yml` will be generated. This file should also be user-specific (see below on how to auto-generate this file by the build toolchain).
5. Look into `reccmp-project.yml` to see what the target is called.
6. Run `reccmp-reccmp.exe --target <YOURTARGET>` from the same directory. If all goes well, you will see match percentage of the function you annotated above.

### Fresh project

1. (Recommended) Set up and activate a virtual Python environment in the directory of your recompilation project (this is different for different operating systems and shells)
2. Install `reccmp`: `pip install https://github.com/isledecomp/reccmp`
3. Run `reccmp-project.exe create --originals path/to/original/binary --cmake-project`
...
5. Look into `reccmp-project.yml` to see what the target is called.
6. Navigate to the build directory and run `reccmp-reccmp.exe --target <YOURTARGET>`.
